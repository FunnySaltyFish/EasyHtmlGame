class FailureScene extends Phaser.Scene{
    ...
    create(){
        ...
        reStartBtn.setSize(reStartBtn.width/2,reStartBtn.height/2);
        resetToCenter(reStartBtn,0,100)
        //开启元件点击监听，并添加监听事件
        
        reStartBtn.once('pointerdown', (pointer)=> {
            reStartBtn.setTint(0x00ff00);
            //console.log("clicked");
            this.scene.start(SCENE_START);
        });
    
        reStartBtn.once('pointerout', function (pointer) {
            this.clearTint();
        });
    
        reStartBtn.once('pointerup', function (pointer) {
            this.clearTint();
        });
