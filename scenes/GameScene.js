...
class GameScene extends Phaser.Scene{
  ...
  preload() {
    ...
    this.load.spritesheet(Keys.player, "assets/role.png", {
      frameWidth: 32,
      frameHeight: 48,
      //   spacing : 45,
      //   margin:6
  ...
  create() {
    ...
    this.anims.create({
      key: Keys.anim_right,
      frames: this.anims.generateFrameNumbers(Keys.player, { start: 8, end: 11 }),
      frameRate: 10,
      repeat: -1,
    });
    ...
    this.anims.create({
      key: Keys.anim_turn,
      frames: [{ key: Keys.player, frame: 0 }],
      frameRate: 20,
    });

    this.anims.create({
      key: Keys.anim_left,
      frames: this.anims.generateFrameNumbers(Keys.player, { start: 4, end: 7 }),
      frameRate: 10,
      repeat: -1,
    });
  ...
  update(){
    ...
    if(score>=gameConfig.targetScore){
      scoreText.setFill("#00ff00")
    }else scoreText.setFill("#000000")
...
function hitBomb(player, bomb) {
  ...
  setTimeout(()=>{
    if(score>=gameConfig.targetScore){
      this.scene.start(SCENE_WIN,{score:score});
    }else this.scene.start(SCENE_FAILURE);
    gameOver = false;
    score = 0;
    currentState = 0;
  },1000);
